            ...
            <table className="table table-zebra w-full">
                ...
                <tbody>
                  {users.map((user,index)=><tr key={user._id}>
                    <th>{index+1}</th>
                    <td className="text-[#C6A921] text-xl font-semibold rounded-xl">{user.userName}</td>
                    <td className="text-[#C6A921] text-xl font-semibold">{user.email}</td>
                   <td className="text-[#C6A921] text-xl font-semibold">
                   {user.role ==='admin' ? 'Admin':<button onClick={()=>handleMakeAdmin(user)} className="btn btn-lg bg-[#C6A921] hover:bg-[#C6A921] ">
